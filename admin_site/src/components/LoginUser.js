...
const LoginUser = (props) => {
  ...
  const Logincheck = async () => {
    ...
    try {
  const response = await axios.post('http://127.0.0.1:8000/login_admin', postData);
    ...
    <div className='login-container'>
      ...
      <div>
        ...
        <button className="login-button" onClick={async () => {
          ...
          try {
            const resp = await axios.post('http://127.0.0.1:8000/register_admin', postData);
            if (resp.data && resp.data.status === 'ok') {
              alert('Admin registrado: ' + resp.data.email);
            }
          } catch (e) {
            console.error('Error registering admin', e);
            alert('Error registrando admin: ' + (e.response?.data?.detail || e.message));
          }
