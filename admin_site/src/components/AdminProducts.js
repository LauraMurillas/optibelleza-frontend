...
const AdminProducts = React.memo((props) => {
  ...
  const response = await axios.get(`http://127.0.0.1:8000/delete_shoes/${id}`, { headers });
  ...
  const updateRow = async (id, category, type, name, price, stock, description) => {
    ...
    try {
      ...
      props.set_active_loader(true)
  const response = await axios.put(`http://127.0.0.1:8000/updateshoes/${id}`, updated_data, { headers });
      props.set_active_loader(false)
  ...
  const addshoes = async (shoes_name, selectedFile, shoes_price, shoes_type, shoes_for, stock, shoes_description, imageSrc) => {
    ...
    try {
      ...
      props.set_active_loader(true)
  const response_image_link = await axios.post('http://127.0.0.1:8000/createshoesimagelink', form_data, { headers });
  ...
  const response = await axios.post('http://127.0.0.1:8000/createshoes', create_shoes_data, { headers: headers1 });
